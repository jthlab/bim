...
class PConst:
    ...
    def exp_integral(self, c: float = 1.0):
        ...
        exp_integrals += gmpy2.fsum(
            [gmpy2.exp(-I[i]) * -gmpy2.expm1(-a[i] * dt[i]) / a[i] for i in range(ndt)]
        )
...
class bSFS:
    ...
    def __init__(self, N, eta=PConst(a=[1.0], t=[0.0]), rho1=0.0, rho2=0.0):
        ...
        def L(beta, SFS):
            ...
            model = self.Ebl(beta)
            return jnp.reshape(
                -jnp.mean(SFS * jnp.log(model)) + pen1 * rho1 + pen2 * rho2, ()
            )
...
class bTree:
    ...
    def __init__(self, N, rho1=0.0, rho2=0.0, frac=None, log_pdf=logfs, abeta=0.0):
        ...
        def obj_bb(beta, n, k, w):
            ...
            return jnp.reshape(
                -loglik + rho1 * abeta + rho2 * jnp.power(abeta, 2) / 2, ()
            )
        ...
        self.rho1 = rho1
        self.rho2 = rho2
        self.cfinder = re.compile(
            "\(\d+\,\d+\)"
        )  # find the merges from the newick tree
        self.success = None
        self.abeta = abeta
    ...
    def predict(self, n, k, w=None, x0=0.0):
        """
    ...
    def split_predict(self, Tree, w=None, x0=0.0):
        """
        ...
        if isinstance(Tree, tskit.trees.Tree):
            ...
            raise ValueError(
                "Not an accepted type. Newick should be a `str` and Tree should be a `tskit.trees.Tree`. Not ",
                type(Tree),
            )
