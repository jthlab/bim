...
def logfr(n, k, beta):
    ...
    return (
        jnp.where(k >= l, logBB(l, n - k, beta, g), logBB(l, k, beta, g)) - normalizer
    )
...
class Neutrality_Tests:
    ...
    def Norm4(self):
        ...
        T3 = (
            2
            * (36 * N**2 * (2 * N + 1) * bn1 - 116 * N**3 + 9 * N**2 + 2 * N - 3)
            / (9 * N * (N - 1) ** 2)
        )
        T4 = 4 / (N * (N - 1) * an)
...
class InferEta:
    ...
    def __init__(self, N, t, a1=0, a2=0, ar=0):
        ...
        def f(A, SFS):
            ...
            loss = loss1v + a1 * l1(logy) + 0.5 * a2 * l2(logy) + 0.5 * ar * lr(logy)
...
def slidingSFS(ts, wsz=None, tsz=None, ssz=None):
    ...
    if nones == 0:
        raise ValueError(
            "you should specift either window size (wsz) or tree-seq size (tsz), not both"
        )
    elif nones == 2:
        sfs = ts.allele_frequency_spectrum(
            windows=[0, L], span_normalise=False, polarised=True
        )[:, 1:-1].astype("int")
        ...
        win[-1] = len(bps) - 1
        win = bps[win]
        sfsp = ts.allele_frequency_spectrum(
            windows=win, span_normalise=False, polarised=True
        )[:, 1:-1].astype("int")
        ...
        starts = [
            np.array(starts)[np.arange(i, len(starts), step, dtype="int")]
            for i in range(step)
        ]
        ends = [
            np.array(ends)[np.arange(i, len(ends), step, dtype="int")]
            for i in range(step)
        ]
...
def intersect_with_weights(bs, be, cps, cpe, y):
    ...
    while (i < n) & (k < m):
        if (bs[i] >= cps[k]) and (be[i] <= cpe[k]):  # all tree inside the region
            ...
            (bs[i] < cps[k]) and (be[i] <= cpe[k]) and (be[i] > cps[k])
        ):  # left intersect with the region
            w = be[i] - cps[k]
            dx_new.append(
                {
                    "start": cps[k],
            ...
            (bs[i] >= cps[k]) and (be[i] > cpe[k]) and (bs[i] <= cpe[k])
        ):  # right intersect with the region
            w = cpe[k] - bs[i]
            dx_new.append(
                {
                    "start": cps[k],
...
def tree_to_splits(Tree, abeta=0):
    ...
    for i in Tree.nodes(order="timeasc"):
        ...
        if lenc == 0:  # leaf
            ...
            sp = split_polytomy(
                masses, abeta
            )  # it returns the random split parent-chilren masses
